...
File change detection via SHA-256 fingerprinting for [Agent Monitors](https://agentmonitors.io).
One of the observation sources bundled with Agent Monitors — select it in a `MONITOR.md`'s
`watch.type` field, or scaffold one with:

```sh
agentmonitors init <name> --type file-fingerprint
```
