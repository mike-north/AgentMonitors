...
Both scaffold a ready-to-run `command-poll` monitor over the GitHub CLI, automatically scoped to the repository the session is operating in via an explicit `cwd:` written at scaffold time rather than a hardcoded `--repo`, and to the current `gh` identity via `--author @me` rather than a scaffolded username. Turning on PR alerting takes one command instead of hand-writing `gh` argv.
