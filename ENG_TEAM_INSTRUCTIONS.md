...
3. **Deadlines in titles are real.** An issue dated (e.g. "due 2026-06-18") outranks undated
   ...
   order.
4. **Blocked or descoping?** Comment on the issue with what/why and remove the `in progress`
   label so the queue reflects reality. Never go silent on a claimed issue.

## Building

5. **Specs govern.** Consult the `docs/specs/` doc matching your task (map in `CLAUDE.md`).
...
9. **Reference issues with `Refs #N`, never `Closes #N`/`Fixes #N`** — unless the PR
...
10. **The PM agent reviews PRs.** Auto-merge on green CI is fine **unless a review with
    ...
    someone else pays.
11. **Comment the PR link on the issue** when you open it; close the issue only when the
    change is merged and the acceptance criteria are demonstrably met.

## Hard rules

12. **Never touch Version PRs** (branch `changeset-release/main`, title "Release packages"):
...
13. **Never flip repo visibility, publish packages locally, or add/modify repo secrets.**
    Releases happen only through the CI pipeline via Mike's Version-PR merge.
14. **No internal codenames/wave numbers in public-facing content** (published packages,
