...
**AP8: The daemon and any process it spawns must never outlive their purpose and must always be reap-able.** Every child the daemon (or a source, adapter, or transport it drives) spawns must be bounded and cleanable — and its liveness must not depend on the daemon staying alive to clean it up. Because Agent Monitors' entire value is running unattended in the background, a process that leaks or orphans on an abrupt daemon death (`kill -9`, crash, OOM) is a reliability defect, not a corner case: with nothing left to reap it, it accumulates until the user must restart. Cleanup that lives only in a parent which can die first is therefore insufficient by construction. This principle is stated as the invariant to hold; per [PP7](#2-product-principles) the pieces are at different maturity, and callers must not assume the whole is current:

- **Current guarantee (POSIX `command-poll`):** a spawned child that could outlive the daemon carries its own OS-level bound — an independent, `detached` self-watchdog that reaps the command's whole process group at a backstop deadline even if the daemon dies first (issue #470, [003 §11.2](./003-source-plugins.md#112-execution-model)). This is the one path where the "liveness must not depend on the daemon" clause is currently met end to end, **for a group whose members all hold a copy of the watchdog's liveness fd** — see the qualification immediately below, and [003 §11.2](./003-source-plugins.md#112-execution-model) for the full analysis, for the one class of descendant this does not currently cover. The guarantee also has a binary precondition: the watchdog itself needs `mkfifo`, `sh`, and `sleep` on `PATH` for every POSIX `command-poll` execution, and fails the command closed (an execution failure, not an unbounded run) whenever one is missing — see [003 §11.2](./003-source-plugins.md#112-execution-model) for the consequence on binary-minimal (slim/distroless/busybox) images.
- **Known gap in the current guarantee:** a descendant the monitored command spawns via a process API that defaults to close-on-exec for non-explicit fds (the default behavior of most modern high-level spawn APIs, not a hardening edge case) never inherits the watchdog's liveness fd, so if the leader that spawned it exits, the watchdog can disarm while that descendant is still alive — reintroducing the #470 orphan failure for that specific class of command. Closing this fully would need a liveness proof independent of fd inheritance (e.g. periodic `kill -0`-based re-verification), which reintroduces a bounded form of the recycled-pgid hazard the fd-based design exists to avoid; left open rather than "fixed" with a different safety property (issue #472 second-round review).
- **Target work:** the same self-bounding on **Windows** (currently bounded only by daemon-resident `taskkill` timers — a documented platform limitation, since Windows has no process groups or portable in-group watchdog); the daemon **sweeping strays left by a prior instance on startup**; and graceful shutdown **actively reaping** in-flight children (the current stop path unwinds the tick loop but does not itself hunt down and kill a still-live in-flight command's descendants). The general cross-subsystem stray-reaping surface — across daemons, channel servers, and sockets — is issue #426.
