...
describe('source-schedule', () => {
  ...
  it('fires an observation when called', async () => {
    const result = await source.observe({ cron: '0 9 * * 1-5' });

    expect(result).toHaveLength(1);
    expect(result[0]?.title).toContain('0 9 * * 1-5');
  });
  ...
  it('uses label as title when provided', async () => {
    ...
    expect(result[0]?.title).toBe('Daily standup reminder');
  });
  ...
  it('defaults timezone to UTC', async () => {
    const result = await source.observe({ cron: '* * * * *' });

    const snap = result[0]?.snapshot as { timezone: string };
    expect(snap.timezone).toBe('UTC');
  });

  it('throws on missing cron config', () => {
    expect(() => source.observe({})).toThrow('cron');
  });
});
