...
| `json-diff` on non-JSON `api-poll` bodies warns before text fallback   | Covered (`plugins/source-api-poll/src/index.test.ts` — "json-diff: warns and falls back to text comparison for non-JSON bodies", "text-diff: does not warn for non-JSON bodies"; `apps/cli/src/commands/cli.integration.test.ts` — "warns when api-poll json-diff receives a non-JSON response body") |
