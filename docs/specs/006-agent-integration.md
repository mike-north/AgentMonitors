...
truncated at 128 code points with a trailing `…`. `--debug` is opt-in, but that must not make it
acceptable to leak a hostile payload to the operator's terminal/logs raw — both paths share one
rendering function so they cannot drift.
