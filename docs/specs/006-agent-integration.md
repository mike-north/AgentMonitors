...
the claim's advisory `message` (the same line `hook claim` surfaces) verbatim into
`hookSpecificOutput.additionalContext`, so a default (`normal`-urgency) monitor produces a visible
